#@title Importation
#This training script's built based on the training code provided by our collegues Oscar and Aggie.
...
no_epochs = 150
k_folds = 5
test_set_directory='./test_dataset/'
model_pth = './model_pth/'
...
model=PRORED(config_vit, img_size=256, num_classes=7).cuda()
...
for fold in range(0,k_folds):
    ...
    for epoch in range(no_epochs):
        ...
        for batch_data in trainloaderCV:
            ...
            loss=loss_function(outputs_sum,labels)
        ...
        if (epoch + 1) % val_interval == 0:
          ...
          with torch.no_grad():
              ...
              for val_data in evalloaderCV:
                  ...
                  val_output_sum=0
                  for val_subout in val_outputs:
                    val_output_sum +=val_subout

                  val_outputs = torch.argmax(val_output_sum, dim = 1)
                  val_outputs = val_outputs.detach().cpu().numpy()
                
                  metrics_pre = []
                  for j in range(1,7):
   
                      a = get_individual_classes(val_outputs, j)
                      b = get_individual_classes(val_labels, j)
                      metrics_pre.append(dice(a,b))
              ...
              if len(best_metrics) == 0:
                   ...
                   torch.save(model.state_dict(), os.path.join(model_pth, "final_metric_model_{}.pth".format(fold)))
              else:
