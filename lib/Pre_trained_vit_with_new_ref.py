...
class ref_module(nn.Module):
    def __init__(self, in_channel, out_channel):
        super(ref_module, self).__init__()
        self.ra1_deco1 = RA(in_channel,out_channel)
        self.ConvDf_1x1_1 = nn.Conv2d(int(out_channel ), 2, kernel_size=1, stride=1, padding=0)

        self.convert = nn.Conv2d(in_channel, out_channel , 1)
        self.bn = nn.BatchNorm2d(out_channel)
        self.relu = nn.ReLU(True)

        self.ConvDf_1x1_2 = nn.Conv2d(int(out_channel ), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_1 = SelFuseFeature(int(out_channel ), auxseg=False, shift_n=5)
    def forward(self, x, y):
      ra = self.ra1_deco1(x,y)
      # print('ra shape',ra.shape)

      conv_x = self.relu(self.bn(self.convert(x)))
      conv_x = F.interpolate(conv_x, scale_factor=2, mode='bilinear', align_corners=True)
      df_rf = self.ConvDf_1x1_1(conv_x)
      df_rf = self.SelDF_1(conv_x,df_rf)
      d1, auxseg = df_rf[:2]
      # print('d1 shape',d1.shape)

      df_rf_1 = self.ConvDf_1x1_1(y)
      df_rf_1 = self.SelDF_1(y,df_rf_1)
      d2, auxseg = df_rf_1[:2]
      # print('d2 shape',d2.shape)

      final = ra.mul(d1).mul(d2)
...
class PRORED(nn.Module):
    def __init__(self, config, img_size=256, num_classes=21843, zero_head=False, vis=False):
        ...
        self.decoder4 = DecoderBottleneck(32, num_classes)
        ...
        # self.ref1_module = ref_module(256,64)
        # self.ref2_module = ref_module(64,32)
        # self.ref3_module = ref_module(32,8)
        ...
        self.SelDF_1 = SelFuseFeature(int(256), auxseg=False, shift_n=6)
        
        self.ConvDf_1x1_2 = nn.Conv2d(int(64), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_2 = SelFuseFeature(int(64), auxseg=False, shift_n=6)
        
        self.ConvDf_1x1_3 = nn.Conv2d(int(32), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_3 = SelFuseFeature(int(32), auxseg=False, shift_n=6)
        
        # self.ConvDf_1x1_4 = nn.Conv2d(int(num_classes), 2, kernel_size=1, stride=1, padding=0)
        # self.SelDF_4 = SelFuseFeature(int(num_classes), auxseg=False, shift_n=6)
    ...
    def forward(self, x, im_size=256):
        ...
        deco1 = self.decoder1(x, x3)
        

        df_1 = self.ConvDf_1x1_1(deco1)
        d1_auxseg = self.SelDF_1(deco1, df_1)
        d1_ref, auxseg = d1_auxseg[:2]
        map1=self.upscore1(d1_ref)
        map1=self.outconv1(map1)
  

        deco2 = self.decoder2(d1_ref, x2)
        df_2 = self.ConvDf_1x1_2(deco2)
        d2_auxseg = self.SelDF_2(deco2,df_2)
        d2_ref,auxseg = d2_auxseg[:2]
        map2=self.upscore2(d2_ref)
        map2=self.outconv2(map2)
        

        deco3 = self.decoder3(d2_ref, x1)
        df_3 = self.ConvDf_1x1_3(deco3)
        d3_auxseg = self.SelDF_3(deco3,df_3)
        d3_ref,auxseg = d3_auxseg[:2]
        map3=self.upscore3(d3_ref)
        map3=self.outconv3(map3)

        deco4 = self.decoder4(d3_ref)
...
if __name__ == '__main__':
    ...
    model = PRORED(config_vit, img_size=256, num_classes=4).cuda()
    model.load_from(weights=np.load(config_vit.pretrained_path))
    input_tensor = torch.randn(1, 1, 256, 256).cuda()
    
    map2,map3,map4=model(input_tensor)
