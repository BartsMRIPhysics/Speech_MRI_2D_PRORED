...
def res2net152_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        model_state = torch.load('/home/ying/Downloads/res2net152_v1b_26w_4s-52fe33d0.pth')
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # x1 torch.Size([4, 128, 64, 64])
        # x2 torch.Size([4, 256, 32, 32])
        # x3 torch.Size([4, 512, 16, 16])
...
class Decoder(nn.Module):
    def __init__(self, out_channels, class_num):
        ...
        self.SelDF_4 = SelFuseFeature(int(out_channels * 4), auxseg=False, shift_n=3)
        
        self.ConvDf_1x1_3= nn.Conv2d(int(out_channels * 2), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_3 = SelFuseFeature(int(out_channels * 2), auxseg=False, shift_n=3)
        
        
        self.ConvDf_1x1_2 = nn.Conv2d(int(out_channels ), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_2 = SelFuseFeature(int(out_channels ), auxseg=False, shift_n=3)
        
        
        self.ConvDf_1x1_1 = nn.Conv2d(int(out_channels * 1 / 2), 2, kernel_size=1, stride=1, padding=0)
        self.SelDF_1 = SelFuseFeature(int(out_channels * 1 / 2), auxseg=False, shift_n=3)
        ...
        self.SelDF = SelFuseFeature(int(out_channels * 1 / 8), auxseg=False, shift_n=3)
        ...
        self.outconv2 = nn.Conv2d(64*2,4,3,padding=1)
        self.outconv1 = nn.Conv2d(16*2,4,3,padding=1)

    def forward(self, x, x1, x2, x3):
        # df = self.ConvDf_1x1_4(x)
        # d2_auxseg = self.SelDF_4(x, df)
        # d2, auxseg = d2_auxseg[:2]
        x=x
        # print('x1 shape',x.shape)
        map1=self.outconv5(x)
        # print('map11 shape',map1.shape)
        map1=self.upscore5(map1)
        # print('map1 shape',map1.shape)

        x = self.decoder1(x, x3)
        # print('x2 shape',x.shape)
        df = self.ConvDf_1x1_3(x)
        # d2_auxseg = self.SelDF_3(x, df)
        # d2, auxseg = d2_auxseg[:2]
        x=x
        # print('x3 deco1 shape',x3.shape)
        ...
        df = self.ConvDf_1x1_2(x)
        # d2_auxseg = self.SelDF_2(x, df)
        # d2, auxseg = d2_auxseg[:2]
        x=x
        ...
        df = self.ConvDf_1x1_1(x)
        # d2_auxseg = self.SelDF_1(x, df)
        # d2, auxseg = d2_auxseg[:2]
        x=x
        ...
        return x, map1,map2,map3,map4,map5
        # return x
...
class TransUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('x2 shape',x2.shape)
        # print('x3 shape',x3.shape)
        x, map1,map2,map3,map4,map5= self.decoder(x, x1, x2, x3)
        # x= self.decoder(x, x1, x2, x3)
